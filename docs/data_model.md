...
### Protocol (`bio.lexicons.temp.surveyProtocol`)

A Protocol defines the contents of a Survey, like what participants should be looking for and what fields they need to fill out.
...
|`requiredFields`|enum\<"eventDuration"\>|optional|List of fields required to complete the survey.|
...
A potential subject for the Survey, e.g. species to be counted. Belongs to a single Protocol, and a Protocol can have many Survey Targets. This might be more appropriate nested within the Protocol.
...
|`samplingPerformedBy`|array\<string\>|optional|Array of DIDs of users (agents) who participated in the survey in addition to the owner (sensu [Humbold Extension](http://rs.tdwg.org/eco/terms/samplingPerformedBy)).|
...
|`eventDurationUnit`|string|optional|Unit of the `eventDurationValue` (sensu [Humbold Extension](http://rs.tdwg.org/eco/terms/eventDurationUnit)). Known values: "minutes", "hours", "days".|
|`eventDurationValue`|integer|optional|Duration of the survey in units specified by `eventDurationUnit` (sensu [Humbold Extension](http://rs.tdwg.org/eco/terms/eventDurationValue)).|
|`location`|org.atgeo.place|required|Geographic location where the survey was conducted. Note that the DwC-CM|
...
|`individualCount`|integer|optional|The number of individuals present at the time of the Occurrence (sensu [DarwinCore](http://rs.tdwg.org/dwc/terms/individualCount))|
